...
public class ExpressionBT {
	...
	public int evaluate() {
		...
		if(type == variable)  // when the type is a variable it is not possible to evaluate
		{
			System.out.print("It is not possible to evaluate");
			return 0;
		}
		else if(operator =='+' ||operator =='-' ||operator =='/' ||operator =='*'  ||operator =='^') // when it has an operator then we calculate right and left from the operator
		{
			int L = left.evaluate();
		...
		else if((type.compareTo("value")==0))   // return the value
		{
			return getValue();
		}
		return 0;
	...
	public void print()
	{
		...
		if (left != null)  // iterate though the left first
		{
			left.print();
			S.push(left);   // then push the left
			if (right != null)
			{
				right.print(); // iterate though the right branch
				S.push(right); //then pushh the right
			}
		}

		else
		{
			System.out.print(type); // when there are no more right and left to push we print the root
			while(!S.isEmpty())// and while the BTStack is not empty we print the value that we pop
			{
